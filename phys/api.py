...
def analyze_jet_plume(amb_fluid, rel_fluid, orif_diam, solver_model,
    ...
    jet_obj = _jet.Jet(rel_fluid, orifice, amb_fluid, solver_model, theta0=rel_angle,
                       nn_conserve_momentum=nozzle_cons_momentum, nn_T=nozzle_t_param, verbose=verbose)
    print ("jet_obj", jet_obj)

    xs, ys, mole_fracs, mass_fracs, vs, temps, rho, B = jet_obj.get_contour_data()
    ...
    if create_plot:
        ...
        #plot_fig = jet_obj.plot_massFrac_Contour(xlims=xlims, ylims=ylims, plot_title=plot_title, mark=contours,
        #                                         vmin=contour_min, vmax=contour_max)
    ...
    result_dict = {'xs': xs, 'ys': ys, 'mole_fracs': mole_fracs, 'mass_fracs': mass_fracs, 'vs': vs,
                   'temps': temps, 'plot': plot_filepath, 'mass_flow_rate': mass_flow_rate, 'rho': rho, 'B': B}
    return result_dict
