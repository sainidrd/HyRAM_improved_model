...
class CoolPropWrapper:
    ...
    def P(self, T, rho):
        ...
        try:
            self.phase = self._cp.PhaseSI('D', rho, 'P', P, self.spec,'PR::Hydrogen')
        except ValueError:
            if (Q < 1) and (Q > 0):
                self.phase = 'twophase'
            elif Q == 1:
    ...
    def rho(self, T, P):
        ...
        try:
            ...
            rho = self._cp.PropsSI('D','P', P,'T', T , 'PR::Hydrogen') #dsaini Use SRK Model
    ...
    def rho_P(self, T, phase):
        ...
        (rho, P): tuple of floats 
            ...
            P - pressure (Pa)
        '''
        rho, P = self._cp.PropsSI(['D', 'P'], 'T', T, 'Q', {'gas':1, 'liquid':0}[phase], self.spec,'PR::Hydrogen')
        self.phase = phase
        return rho, P
    ...
    def rho_T(self, P, phase):
        ...
        (rho, P): tuple of floats 
            ...
            P - pressure (Pa)
        '''
        rho, T = self._cp.PropsSI(['D', 'T'], 'P', P, 'Q', {'gas':1, 'liquid':0}[phase], self.spec,'PR::Hydrogen')
        self.phase = phase
        return rho, T
    ...
    def P_T(self, rho, phase):
        ...
        (rho, P): tuple of floats 
            ...
            P - pressure (Pa)
        '''
        P, T = self._cp.PropsSI(['P', 'T'], 'D', rho, 'Q', {'gas':1, 'liquid':0}[phase], self.spec,'PR::Hydrogen')
        self.phase = phase
        return P, T
    ...
    def _err_H(self, T1, P1, v1, T2, P2, v2, usePhase1 = False, usePhase2 = False):
        ...
        '''
        try:
            h1 = self._cp.PropsSI('H', 'T', T1, 'P', P1, self.spec,'PR::Hydrogen')
        except:
            print('tp')
            h1 = self._cp.PropsSI('H', 'T|'+self.phase, T1, 'P', P1, self.spec,'PR::Hydrogen')
        try:
            h2 = self._cp.PropsSI('H', 'T', T2, 'P', P2, self.spec)
        except:
            print('tp')
            h2 = self._cp.PropsSI('H', 'T|'+self.phase, T2, 'P', P2, self.spec,'PR::Hydrogen')
    ...
    def _X(self, Y, other = 'air'):
        MW = self._cp.PropsSI('M', self.spec,'PR::Hydrogen')
        MW_other = self._cp.PropsSI('M', other,'PR::Hydrogen')
    ...
    def a(self, T = None, P = None, S = None):
        ...
        def a_2phase(T, Q):
            ...
            [al, rhol], [av, rhov] = self._cp.PropsSI(['A', 'D'], 'P', P, 'Q', [0, 1], self.spec,'PR::Hydrogen')
    ...
    def PropsSI(self, output, **kwargs):
        ...
        try:
            ...
            out = self._cp.PropsSI(output, k1, v1, k2, v2, self.spec,'PR::Hydrogen')
            return out
        except ValueError:
            if ('T' in kwargs) and ('D' in kwargs):
                ...
                def err(P):
                    ...
                    #return D - self._cp.PropsSI('D', 'T', T, 'P', P, self.spec,'PR::Hydrogen')
                ...
                Tmin = self._cp.PropsSI('Tmin', self.spec,'PR::Hydrogen')
                def err(T):
                    #return D - self._cp.PropsSI('D', 'T', max(T, Tmin), 'P', P, self.spec)
                    return D - self._cp.PropsSI('D', 'T', max(T, Tmin), 'P', P, self.spec,'PR::Hydrogen')
                #dT = 5
                ...
                def err(T):
                    err = v - self._cp.PropsSI(k, 'T', T, 'P', P, self.spec,'PR::Hydrogen')
                    return err
                ...
                k, v = list(kwargs.items())[0]
                def err(P):
                    return v - self._cp.PropsSI(k, 'T', T, 'P', P, self.spec,'PR::Hydrogen')
    ...
    def s(self, T = None, P = None, rho = None, phase = None):
        ...
        if T is not None and P is not None:
            return self._cp.PropsSI('S', 'T' + str, T, 'P', P, self.spec,'PR::Hydrogen')
        elif T is not None and rho is not None:
            return self._cp.PropsSI('S', 'T' + str, T, 'D', rho, self.spec,'PR::Hydrogen')
        elif P is not None and rho is not None:
            return self._cp.PropsSI('S', 'D' + str, rho, 'P', P, self.spec,'PR::Hydrogen')
        else:
            raise warnings.warn('system not properly defined')
